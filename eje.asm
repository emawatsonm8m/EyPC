...
entrada_tecla   db      0		;Tecla ingresada por el usuario 
...
max_misiles_j equ 6
mis_j_activos db 0
mis_j_col db max_misiles_j dup (0)
mis_j_ren db max_misiles_j dup (0)
mis_j_vivos db max_misiles_j dup (0) ;0=libres 1=activo

;======== Misiles del enemigo ===========
max_misiles_e equ 6
mis_e_activos db 0
mis_e_col db max_misiles_e dup (0)
mis_e_ren db max_misiles_e dup (0)
mis_e_vivos db max_misiles_e dup (0) 

;Temporizadores de misiles y la parte de hacerlo aleatorio
t_mis_j dw 0 
t_mis_e dw 0 
t_ultimo_disparo_j dw 0

seed dw 0ACE1h

;mensaje de juego terminado
msg_termina db "GAME OVER"
...
inicio:					;etiqueta inicio
	...
	cmp ax,0FFFFh		;compara el valor de AX con FFFFh, si el resultado es zero, entonces existe el driver de mouse
...
continuar:
    ...
    call MISILES_JUGADOR
    call MISILES_ENEMIGO
    call COLISION_MJ_ENE
    call COLISION_ME_JUG
...
loop_pause:
    ; en pausa solo puedes usar entradas y botones
    call LEER_TECLADO
    jmp comenzar_juego
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    ;Dibujar el misil a aventar del jugador 'J'
    Dibujar_Misil_J proc
      posiciona_cursor [ren_aux], [col_aux]
    ...
    ;Dibujar el misil del enemigo computadora 'E'
    Dibujar_Misil_E proc
      posiciona_cursor [ren_aux], [col_aux]
      imprime_caracter_color 9,cAzulClaro, bgNegro
      ret
    endp
...
MOV_NAVE proc
    ...
    cmp al,tecla_enter
    je intento_disparo_jugador

    ret

intento_disparo_jugador:
    ; leer tiempo actual
    mov ah, 00h
    int 1Ah
    mov ax, dx

    ; calcular diferencia desde el último disparo
    sub ax, [t_ultimo_disparo_j]
    cmp ax, 15              
    jb limite_alcanzado     ; si no ha pasado → NO dispara

    ; actualizar tiempo del último disparo
    mov [t_ultimo_disparo_j], dx

    jmp disparar_jugador    ; ya se puede disparar

; --- disparo manual con Enter ---
disparar_jugador:
    mov si,0
Buscar_Slot_J:
    cmp si,max_misiles_j
    je limite_alcanzado
    mov al,[mis_j_vivos+si]
    cmp al,0
    jne slot_sig_j

    mov [mis_j_vivos+si],1
    inc [mis_j_activos]
    mov al,[player_col]
    mov [mis_j_col+si],al
    
    ; *** CORRECCIÓN APLICADA AQUÍ ***
    ; Restamos 3 a player_ren para que el misil empiece por encima de la nave (3 renglones de alto)
    mov al,[player_ren]
    sub al,3               
    mov [mis_j_ren+si],al
    
    jmp limite_alcanzado
...
limite_alcanzado:
    mov byte ptr [entrada_tecla],0   ; limpiar tecla para no repetir
    ret
MOV_NAVE endp


; ============================
; Avance de misiles jugador
; ============================

MISILES_JUGADOR proc
    cmp [status],1
    jne fin_mj

    ; --- Lógica de Temporización ---
    mov ah, 00h
    int 1Ah             ; Lee el contador de ticks del sistema (DX:AX)
    mov ax, dx
    sub ax, [t_mis_j]   ; Calcula la diferencia de ticks desde el último movimiento
    cmp ax, 5          ; Verifica si han pasado 5 ticks (aprox. 1 segundo)
    jb fin_mj           ; Si no han pasado, salta al final (no mueve los misiles)
    mov [t_mis_j], dx   ; Si pasaron, actualiza el tiempo de referencia

Avanza_mj:
    mov si,0
Ciclo_mj:
    cmp si,max_misiles_j
    je fin_mj_final     ; Cambiado el nombre de la etiqueta para evitar conflicto
    mov al,[mis_j_vivos+si]
    cmp al,1
    jne sig_mj

    ; 1. borrar posición actual
    mov al,[mis_j_col+si]
    mov [col_aux],al
    mov al,[mis_j_ren+si]
    mov [ren_aux],al
    ...
    ; 2. mover arriba (solo 1 renglón)
    dec byte ptr [mis_j_ren+si]

    ; 3. si se sale del área, liberar
    mov al,[mis_j_ren+si]
    cmp al,lim_superior+1
    jl libera_mj

    ; 4. dibujar nueva posición
    mov al,[mis_j_col+si]
    mov [col_aux],al
    mov al,[mis_j_ren+si]
    mov [ren_aux],al
    call Dibujar_Misil_J
    jmp sig_mj

libera_mj:
    mov byte ptr [mis_j_vivos+si],0
    dec [mis_j_activos]
...
fin_mj_final:
    ret ; Retorno final después del ciclo de misiles
endp
...
MISILES_ENEMIGO proc
  ...
  ;Este es el disparo aleatorio e de cada N
  call RND16
  mov ax, [seed]
  and ax, 0007h
  cmp ax, 0
  ...
  ;creamos el misil si hay slot
  mov si, 0
Buscar_slot_e:
  cmp si, max_misiles_e
  je avanza_me
  mov al, [mis_e_vivos+si]
  cmp al,0
  jne slot_sig_e
  mov [mis_e_vivos+si],1
  inc [mis_e_activos]
  mov al, [enemy_col]
  mov [mis_e_col+si], al
  mov al, [enemy_ren]
...
Ciclo_me:
  ...
  ;lo movemos abajo
  inc byte ptr [mis_e_ren+si]

  ;Por si se sale del área
  mov al, [mis_e_ren+si]
  cmp al, lim_inferior-1
  jg libera_me
  ...
  mov al, [mis_e_ren+si]
  mov [ren_aux], al
  call Dibujar_Misil_E
  jmp sig_me

libera_me:
  mov byte ptr [mis_e_vivos+si], 0
  dec [mis_e_activos]
...
GAME_OVER proc
    ; Comparar y actualizar hiscore si es necesario
    mov ax, [player_score]
    mov bx, [player_hiscore]
    cmp ax, bx
    jbe actualiza_hs
...
actualiza_hs:
    ...
    ; Refrescar scores en pantalla
    call BORRA_SCORES
    call IMPRIME_SCORES

    ; Mostrar mensaje simple de GAME OVER en área de juego
    posiciona_cursor 12, 10
    lea bp, [no_mouse]     
    ; imprime "GAME OVER" usando caracteres individuales:
    posiciona_cursor 12, 30
    imprime_cadena_color msg_termina,9,cRojoClaro,bgNegro

    ; Pausar el juego hasta que el usuario presione play
    mov byte ptr [status], 2

    ; Esperar a que usuario haga clic en play
espera_play:
    lee_mouse
    test bx,0001h
    jz espera_play
    ; convertir mouse a columnas/ren como en mouse handler
    mov ax,dx
    div [ocho]
    mov dx,ax
    mov ax,cx
    div [ocho]
    mov cx,ax
    ; comprobar si clic en el área del botón PLAY 
    cmp dx, play_ren + 1
    jne espera_play
    cmp cx, play_col + 1
    jne espera_play

    ; si le da play se reinicia el juego
    call BORRA_SCORES
    call DATOS_INICIALES

    ; Reiniciar enemigos y misiles
    mov byte ptr [enemy_ren], 3
    mov byte ptr [enemy_col], ini_columna
    call ELIMINA_ENEMIGO
    call IMPRIME_ENEMIGO

    ; Libera todos los misiles
    mov si,0
limpiar_misiles:
    cmp si, max_misiles_j
    je limpiar_misiles
...
limpia_misiles_e:
    mov si,0
limpia_misiles_e_loop:
    cmp si, max_misiles_e
    je desp_limpiar_misiles
    mov byte ptr [mis_e_vivos+si],0
    inc si
    jmp limpia_misiles_e_loop

desp_limpiar_misiles:

    ; Reimprimir UI de scores y lives
    call BORRA_SCORES
    call IMPRIME_SCORES
    call IMPRIME_LIVES

    ; Poner status = 1 para continuar juego
    mov byte ptr [status], 1
...
; Impacto de misiles del jugador contra enemigo
COLISION_MJ_ENE proc
...
Ciclo_cj:
    ...
    ; comparar overlap +-1 columna/ren
    mov al,[mis_j_col+si]
    mov bl,[enemy_col]
    sub bl,1
    cmp al,bl
    ...
    ; impacto: borrar misil y liberar slot
    mov al,[mis_j_col+si]
    mov [col_aux],al
    mov al,[mis_j_ren+si]
    mov [ren_aux],al
    ...
    ; Aumentar score del jugador
    mov bx, [player_score]
    add bx, 1            ; puntaje por enemigo (ajusta si quieres más)
    mov [player_score], bx

    ; Si player_score > player_hiscore = actualizar hiscore
    mov ax, [player_score]
    mov bx, [player_hiscore]
    cmp ax, bx
    jbe no_hiscore
...
no_hiscore:
    ; refrescar marcadores en pantalla (borra y reimprime)
    call BORRA_SCORES
    call IMPRIME_SCORES

    ; Aquí vamos a respawnear la nave en la parte superior
    call ELIMINA_ENEMIGO
    mov byte ptr [enemy_ren], 3
    call RND16
    mov ax, [seed]
    and ax, 00FFh
    ; rango ancho disponible = lim_derecho - lim_izquierdo - 6 (ajusta margen)
    mov bx, lim_derecho
    sub bx, lim_izquierdo
    sub bx, 6
    cmp bx,0
    jle colision
    ; ax % bx
...
desp_colision:
    ...
    ; frena avance vertical por un tiempo (reinicia el temporizador)
    mov ah,00h
    int 1Ah
    mov [t_atacar_enem],dx
...
; Impacto de misiles del enemigo contra jugador
COLISION_ME_JUG proc
    ; si el jugador se muere se sale
    cmp [player_dead],1
    je fin_ce
...
Ciclo_ce:
    ...
    ; overlap +-1 columna/ren con sprite del jugador
    mov al,[mis_e_col+si]
    mov bl,[player_col]
    sub bl,2
    cmp al,bl
    ...
    ; impacto: muere el jugador y se borra el misil
    mov al,[mis_e_col+si]
    mov [col_aux],al
    mov al,[mis_e_ren+si]
    mov [ren_aux],al
...
end inicio			;fin de etiqueta de inicio, fin de programa
